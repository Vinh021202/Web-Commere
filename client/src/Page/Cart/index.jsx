        ...
        <div className="section-shell listing-hero overflow-hidden px-6 py-8 md:px-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-[720px]">
              <span className="eyebrow mb-4">{context?.t('yourCart')}</span>
              <h1 className="section-heading mb-3">{context?.t('cartHeroTitle')}</h1>
              <p className="mb-0 max-w-[620px] text-[14px] text-[rgba(31,41,55,0.72)]">
                {context?.t('cartHeroText')}
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:min-w-[430px]">
              ...
              <div className="soft-card p-4">
                <span className="listing-stat__label">{context?.t('subtotal')}</span>
                <strong className="listing-stat__value !text-[1.3rem]">{context?.formatCurrency(totalAmount)}</strong>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="section-shell overflow-hidden px-0 py-0">
              <div className="flex min-h-[124px] items-center border-b border-[rgba(255,82,82,0.12)] px-6 py-5">
                <div className="flex w-full flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <h2 className="m-0 text-[26px] font-[800] text-[#201714]">{context?.t('cartTitle')}</h2>
                    <p className="mb-0 mt-1 text-[14px] text-[rgba(31,41,55,0.7)]">
                      {context?.t('itemsInCart', { count: totalProducts })}
                    </p>
                  </div>
                  ...
                  >
                    {context?.t('continueShopping')} <FiArrowRight className="text-[16px]" />
                  </Link>
                </div>
              </div>
              ...
              {totalProducts !== 0 ? (
                ...
                <div className="flex flex-col items-center justify-center gap-5 px-6 py-14 text-center">
                  ...
                  <div>
                    <h4 className="text-[24px] font-[800] text-[#201714]">{context?.t('emptyCart')}</h4>
                    <p className="mx-auto mt-2 max-w-[420px] text-[14px] text-[rgba(31,41,55,0.66)]">
                      {context?.t('emptyCartHint')}
                    </p>
                  </div>
                  <Link to="/">
                    <Button className="bg-org btn-lg">{context?.t('continueShopping')}</Button>
                  </Link>
                </div>
              )}
            </div>
            ...
            <div className="section-shell sticky top-6 overflow-hidden px-0 py-0">
              <div className="flex min-h-[124px] items-center border-b border-[rgba(255,82,82,0.12)] px-6 py-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="m-0 text-[24px] font-[800] text-[#201714]">{context?.t('cartTotal')}</h3>
                    <p className="mb-0 mt-1 text-[13px] text-[rgba(31,41,55,0.62)]">{context?.t('cartTotalHint')}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-6">
                <div className="rounded-[28px] border border-[rgba(255,82,82,0.12)] bg-[linear-gradient(180deg,#ffffff_0%,#fff8f5_100%)] p-5 shadow-[0_16px_30px_rgba(15,23,42,0.06)]">
                  <div className="flex items-center justify-between py-3">
                    <span className="text-[14px] font-[700] text-[#6b7280]">{context?.t('subtotal')}</span>
                    <span className="text-[15px] font-[800] text-[#1f2937]">{context?.formatCurrency(totalAmount)}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-[rgba(255,82,82,0.1)] py-3">
                    <span className="text-[14px] font-[700] text-[#6b7280]">{context?.t('deliveryPoint')}</span>
                ...
                <div className="mt-5 space-y-3">
                  <Link to="/checkout" className="block">
                    <Button className="bg-org btn-lg flex w-full gap-2" disabled={totalProducts === 0}>
                      <BsFillBagCheckFill className="text-[20px]" /> {context?.t('checkout')}
                    </Button>
                  </Link>

                  <Link to="/" className="block">
                    <Button className="bg-org btn-border btn-lg w-full">{context?.t('continueBrowsing')}</Button>
                  </Link>
                </div>

                <div className="mt-5 rounded-[22px] border border-[rgba(255,82,82,0.1)] bg-white/75 p-4">
                  <p className="mb-0 text-[12px] leading-6 text-[rgba(31,41,55,0.6)]">{context?.t('keepCartHint')}</p>
                </div>
              </div>
            </div>
          </div>
